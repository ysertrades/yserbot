...
Object.assign(OPS, {
  ...
  async giveawaystart(guildId, body, ctx) {
    try {
      if (!ctx.guild && ctx.client?.guilds?.cache) {
        ctx.guild = ctx.client.guilds.cache.get(guildId) || null;
      }
      if (!ctx.guild) return { error: 'guild_unavailable', detail: 'Bot is not in this server cache — restart the bot and try again.' };
      ...
      if (r.ok) {
        ...
        } catch (annErr) { console.warn('[writes.giveawaystart] announce', annErr.message); }
      }
      return r;
    } catch (err) {
      console.error('[writes.giveawaystart]', err);
      return { error: 'launch_failed', detail: err.message || String(err) };
    }
  },
  ...
  async autorole(guildId, body, ctx) {
    const r = moderationPanel.saveAutoRole(guildId, body, ctx);
  ...
  async leveling(guildId, body, ctx) {
    try {
      ...
      if (body && body.op === 'reset') {
        ...
        const snap = leveling.resetAllXp(guildId, ctx.session?.uid, guild);
...
Object.assign(OPS, {
  // Bot identity. Global fields are owner-only; nickname is gated by the
  // operator's allowGuildNickname flag (or owner). See web/botProfile.js.
  'bot-profile-global': async (_guildId, body, ctx) => {
    const botProfile = require('./botProfile');
    const auth = require('./auth');
    if (!auth.isOwner(ctx.session.uid)) return { error: 'forbidden' };
    return botProfile.applyGlobal(body || {}, ctx.client);
  },
