...
async function route(req, res, client) {
  ...
  if (p.startsWith('/api/')) {
    ...
    if (p === '/api/me') {
      // Do NOT block on client.isReady() — that caused "Bot not responding"
      // while Discord was still connecting. Empty guilds is acceptable.
    ...
    if (p === '/api/leaderboard') {
      return json(res, 200, await api.leaderboard(client, 10, url.searchParams.get('g')));
    }
    ...
    if (p.startsWith('/api/owner/')) {
      ...
      // Remove the bot from a server. Irreversible without a re-invite.
      if (p === '/api/owner/leave') {
        if (req.method !== 'POST') return json(res, 405, { error: 'use_post' });
        if (!auth.csrfValid(session, req.headers['x-csrf-token'])) return json(res, 403, { error: 'bad_csrf' });
        let body;
        try { body = await readJsonBody(req); }
        catch (err) { return json(res, err.message === 'body_too_large' ? 413 : 400, { error: err.message }); }
        const result = await owner.leaveGuild(String(body.guildId || ''), client);
        if (result.error) return json(res, 400, result);
        return json(res, 200, result);
      }

      // Master switch: whether server owners may set the bot's per-guild nick.
      if (p === '/api/owner/bot-profile') {
        const botProfile = require('./botProfile');
        if (req.method === 'GET') {
          return json(res, 200, botProfile.flags());
        }
        if (req.method !== 'POST') return json(res, 405, { error: 'use_post' });
        if (!auth.csrfValid(session, req.headers['x-csrf-token'])) return json(res, 403, { error: 'bad_csrf' });
        let body;
        ...
        const next = botProfile.setFlags({
          allowGuildNickname: !!body.allowGuildNickname,
        });
    ...
    const guildMatch = /^\/api\/guild\/(\d{5,25})(?:\/([a-z]+(?:-[a-z]+)*))?$/.exec(p);
    if (guildMatch) {
      const [, guildId, op] = guildMatch;
      if (!auth.canAccessGuild(session, guildId, client)) return json(res, 403, { error: 'forbidden' });

      if (!op) return json(res, 200, await api.guildOverview(guildId, client, session));
