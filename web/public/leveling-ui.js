 ...
 * Per-section save · fixed quadratic curve · role ladder · channel unlocks.
 */
(function () {
  ...
  function render() {
    ...
    hero.append(top);
    hero.append(el('p', 'muted', 'Chat XP · fixed curve · roles stack. Premium is Whop only.'));
    ...
    curve.append(el('p', 'hint', 'Fixed formula: XP to go from level n → n+1 is 5·n² + 50·n + 100. Roles stay cumulative.'));
    const preview = el('div', 'lvl-curve-table');
    const table = (L.curveTable && L.curveTable.length)
      ? L.curveTable
      : [1, 5, 15, 30, 50].map(lv => {
          function step(n) { return 5 * n * n + 50 * n + 100; }
          let total = 0;
          for (let n = 0; n < lv; n++) total += step(n);
          return { level: lv, totalXp: total, stepXp: step(lv - 1) };
        });
    for (const row of table) {
      const r = el('div', 'row');
      r.append(el('span', 'k', 'Level ' + row.level));
      r.append(el('span', 'v', fmt(row.totalXp) + ' XP total · ' + fmt(row.stepXp) + ' / step'));
      preview.append(r);
    }
    curve.append(preview);
    curve.append(el('p', 'hint', L.formula || 'xp_to_next(n) = 5·n² + 50·n + 100'));
    root.append(curve);
    ...
    function paintRewards() {
      ...
      rewardDraft.forEach((r, idx) => {
        ...
        function step(n) { return 5 * n * n + 50 * n + 100; }
        let needTotal = 0;
        for (let n = 0; n < lv; n++) needTotal += step(n);
        const xpHint = el('span', 'lvl-xp-need', fmt(needTotal) + ' XP to reach');
    ...
    reset.addEventListener('click', async () => {
      if (typeof askConfirm === 'function') {
        const ok = await askConfirm({
          ...
          message: 'Clears all XP and restores default roles, rates, and channel unlocks.',
