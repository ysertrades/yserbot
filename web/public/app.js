...
function renderBotProfile() {
  ...
  if (bp.canEditPresence) {
    ...
    resetStatus.title = 'Back to Online · Watching YSER Flow | /help';
    resetStatus.addEventListener('click', async () => {
      if (!await askConfirm({
        title: 'Reset bot status?',
        message: 'Puts status back to Online and the activity to “Watching YSER Flow | /help”.',
  ...
  if (bp.canEditGlobal) {
    ...
    const resetBio = el('button', 'btn small', 'Clear bio');
    resetBio.type = 'button';
    resetBio.addEventListener('click', async () => {
      if (!await askConfirm({
        title: 'Clear the bot bio?',
        message: 'Removes the About me text. Username and avatar are left alone.',
        confirmLabel: 'Clear bio',
      })) return;
      resetBio.disabled = true;
      try {
        const res = await post('bot-profile-global', { bio: '' });
        if (res?.ok) {
          try { state.overview = await get(`/api/guild/${state.guildId}`); } catch { /* keep */ }
          renderBotProfile();
        }
      } finally { resetBio.disabled = false; }
    });

    globalActions.append(saveGlobal, resetBio);
