...
function openAvatarStudio(host, img, fileName, onReady) {
  const SIZE = 256; // output pixels (Discord-friendly)
  const VIEW = Math.min(280, Math.floor((window.innerWidth || 320) * 0.72));

  const state = {
    // scale relative to "cover" the circle
    zoom: 1,
    // pan in view pixels
    x: 0,
    y: 0,
  ...
  // Natural cover scale: image fills the circle at zoom=1
  ...
  function draw() {
    ...
    // Center + pan
    const dx = (VIEW - w) / 2 + state.x;
    const dy = (VIEW - h) / 2 + state.y;
  ...
  function bake() {
    // Render the same framing into a SIZE×SIZE circle-safe square JPEG
    const out = document.createElement('canvas');
    out.width = SIZE;
    out.height = SIZE;
    const octx = out.getContext('2d');
    const scale = cover * state.zoom * (SIZE / VIEW);
    const w = img.naturalWidth * scale;
    const h = img.naturalHeight * scale;
    const dx = (SIZE - w) / 2 + state.x * (SIZE / VIEW);
    const dy = (SIZE - h) / 2 + state.y * (SIZE / VIEW);
  ...
  zoomField.style.cssText = 'width:100%;max-width:' + VIEW + 'px';
  ...
  const hint = el('p', 'hint', 'Drag the image to reposition · scroll or use the slider to zoom');
