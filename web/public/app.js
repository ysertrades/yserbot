...
async function post(op, body, { quiet = false } = {}) {
  ...
  else if (data.global && state.overview?.botProfile) {
    // Lightweight profile response without a full overview rebuild.
    state.overview.botProfile.global = { ...state.overview.botProfile.global, ...data.global };
    renderBotProfile();
  }
  return data;
}
...
/* ── bot profile ──────────────────────────────────────────────────────────
   Global identity is operator-only. Per-server nickname is available to guild
   owners only when the operator has allowed it. Same form controls, same post()
   path as every other panel write. */
function renderBotProfile() {
  ...
  top.append(el('span', 'nm', guild.displayName || g.globalName || g.username || 'Bot'));
  preview.append(top);
  if (g.bio) preview.append(el('p', 'hint', g.bio));
  else preview.append(el('p', 'hint', guild.nickname ? `Nickname in this server: ${guild.nickname}` : 'No server nickname set.'));
  nodes.push(preview);

  // Nickname — per server
  const nickDraft = { nickname: guild.nickname || '' };
  if (bp.canEditNickname) {
    nodes.push(textField('Nickname in this server', nickDraft.nickname, v => { nickDraft.nickname = v; }, {
      placeholder: 'Leave empty to use the global name',
    }));
    nodes.push(actions(() => post('bot-profile-nick', { nickname: nickDraft.nickname }), {
      label: 'Save nickname',
    }));
  } else {
    nodes.push(el('p', 'hint', 'Server nickname is locked. The bot operator has not allowed servers to change it.'));
  }

  // Global identity — owner only
  if (bp.canEditGlobal) {
    nodes.push(el('h3', null, 'Global identity'));
    nodes.push(el('p', 'hint', 'These change the bot in every server. Username changes are rate-limited by Discord.'));
    const globalDraft = {
      username: g.username || '',
      bio: g.bio || '',
      avatar: null,       // data URI once a file is picked
      avatarName: '',
      clearAvatar: false,
    };
    ...
    // File upload, not a link — the file is resized client-side then sent as a data URI.
    const avLabel = el('label', 'field');
    avLabel.append(el('span', null, 'Avatar image'));
    const fileInput = el('input');
    fileInput.type = 'file';
    fileInput.accept = 'image/png,image/jpeg,image/webp,image/gif';
    const avHint = el('p', 'hint', 'PNG, JPG, WebP or GIF. Large images are resized automatically.');
    const avPreview = el('img');
    avPreview.alt = '';
    avPreview.style.cssText = 'width:48px;height:48px;border-radius:50%;display:none;margin-top:8px;object-fit:cover';
    fileInput.addEventListener('change', async () => {
      const file = fileInput.files && fileInput.files[0];
      if (!file) {
        globalDraft.avatar = null;
        globalDraft.avatarName = '';
        avPreview.style.display = 'none';
        avHint.textContent = 'PNG, JPG, WebP or GIF. Large images are resized automatically.';
        return;
      }
      try {
        globalDraft.avatar = await readAvatarDataUri(file);
        globalDraft.avatarName = file.name;
        globalDraft.clearAvatar = false;
        avPreview.src = globalDraft.avatar;
        avPreview.style.display = 'block';
        avHint.textContent = file.name + ' · ready to save';
      } catch (err) {
        globalDraft.avatar = null;
    ...
    avLabel.append(fileInput);
    nodes.push(avLabel, avHint, avPreview);

    const clearRow = el('label', 'toggle');
    const clearInput = el('input');
    clearInput.type = 'checkbox';
    clearInput.addEventListener('change', () => {
      globalDraft.clearAvatar = clearInput.checked;
    ...
    clearRow.append(el('span', null, 'Remove current avatar'), clearInput);
    nodes.push(clearRow);
    ...
    nodes.push(actions(async () => {
      ...
      if (globalDraft.clearAvatar) body.avatar = null;
      else if (globalDraft.avatar) body.avatar = globalDraft.avatar;
      await post('bot-profile-global', body);
    }, { label: 'Save global profile' }));
  }
...
function readAvatarDataUri(file) {
  return new Promise((resolve, reject) => {
    if (!file || !file.type || !file.type.startsWith('image/')) {
      reject(new Error('Pick an image file.'));
      return;
    }
    // Hard cap before we even decode — avoids blowing memory on a 20MB photo.
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        // Keep the upload small so the request never stalls: 256px JPEG is
        // enough for a Discord avatar and stays well under the body limit.
        const max = 256;
        let w = img.naturalWidth || img.width;
        let h = img.naturalHeight || img.height;
        if (!w || !h) { reject(new Error('That image has no size.')); return; }
        if (w > max || h > max) {
