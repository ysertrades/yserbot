...
function renderBotProfile() {
  ...
  if (bp.canEditGlobal) {
    ...
    // Upload only — pick the photo you want. No remove toggle.
    const avLabel = el('label', 'field');
    avLabel.append(el('span', null, 'Avatar image'));
    const fileInput = el('input');
    fileInput.type = 'file';
    fileInput.accept = 'image/png,image/jpeg,image/webp,image/gif';
    const avHint = el('p', 'hint', 'Choose the photo you want for the bot. PNG, JPG, WebP or GIF — large images are resized automatically.');
    ...
    fileInput.addEventListener('change', async () => {
      ...
      if (!file) {
        ...
        avHint.textContent = 'Choose the photo you want for the bot. PNG, JPG, WebP or GIF — large images are resized automatically.';
        return;
      }
      try {
  ...
  if (bp.canEditPresence || bp.canEditNickname || bp.canEditGlobal) {
    ...
    nodes.push(el('p', 'hint',
      'Puts status back to Online · Watching YSER Flow | /help, clears this server’s nickname, and clears the bio. Username and avatar are not changed (upload a new photo if you want a different picture).'));
    ...
    resetAll.addEventListener('click', async () => {
      if (!await askConfirm({
        ...
        message: 'Status → Online, activity → “YSER Flow | /help”, this server’s nickname cleared, bio cleared. Username and avatar stay as they are.',
