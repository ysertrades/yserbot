...
function renderBotProfile() {
  ...
  const presence = bp.presence || { status: 'online', activityType: 'watching', activityText: '' };
  ...
  if (bp.canEditPresence) {
    ...
    nodes.push(actions(async () => {
      const res = await post('bot-profile-presence', presenceDraft);
      if (res?.ok && res.presence && state.overview?.botProfile) {
  ...
  if (bp.canEditNickname) {
    ...
    saveNick.addEventListener('click', async () => {
      ...
      try {
        ...
        if (res?.ok && res.guild && state.overview?.botProfile) {
          state.overview.botProfile.guild = { ...state.overview.botProfile.guild, ...res.guild };
          renderBotProfile();
        }
      } finally { saveNick.disabled = false; }
    });
    const clearNick = el('button', 'btn small', 'Clear nickname');
  ...
  if (bp.canEditGlobal) {
    ...
    nodes.push(actions(async () => {
      ...
      // Username changes are permanent-feeling and rate-limited — confirm.
      if (body.username) {
        if (!await askConfirm({
      ...
      if (res?.ok && res.global && state.overview?.botProfile) {
        state.overview.botProfile.global = { ...state.overview.botProfile.global, ...res.global };
        renderBotProfile();
      }
