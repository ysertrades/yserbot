...
/** Streak → payout multiplier. Index = streak length after a win. */
const HL_STREAK_MULT = [1, 1.2, 1.5, 2, 3, 5, 8, 12, 20];
function hlMultiplier(streak) {
  if (streak <= 0) return 1;
  if (streak >= 8) return 20;
  return HL_STREAK_MULT[streak];
}
...
/** Fresh single-deck shoe, shuffled. */
function hlNewDeck() {
  return shuffle(createDeck(1));
}

/**
 * Compare player guess against current → next.
 * guess: 'higher' | 'lower' | 'same'
 */
function hlResolve(current, next, guess) {
  const a = rankValue(current.r);
  const b = rankValue(next.r);
  let outcome = 'same';
  if (b > a) outcome = 'higher';
  else if (b < a) outcome = 'lower';
  return { outcome, won: outcome === guess, a, b };
}

/**
 * Visual: one card (waiting) or current → next with result tint.
 * @param {{r,s}} current
 * @param {{r,s}|null} next
 * @param {'win'|'lose'|null} result
 * @param {number} streak
 * @param {number} mult
 */
function renderHigherLowerPng(current, next = null, result = null, streak = 0, mult = 1) {
  const W = 520, H = 280;
  const png = new PNG({ width: W, height: H });
  _flatBg(png, [12, 16, 28, 255]);
  _glassPanel(png, 16, 16, W - 32, H - 32, { radius: 18, fill: [20, 28, 48, 220], border: [80, 120, 200, 90] });

  // Title strip
  _drawPixelLabel(png, 'HIGHER OR LOWER', W / 2, 42, 2, [180, 200, 255, 255]);
  ...
  if (next) {
    ...
    const y0 = 72;
    _drawCard(png, x0, y0, cardW, cardH, current, false, false);
    // arrow
    _drawPixelLabel(png, '>', x0 + cardW + gap / 2, y0 + cardH / 2, 3,
      result === 'win' ? [80, 220, 140, 255] : result === 'lose' ? [255, 100, 100, 255] : [200, 200, 220, 255]);
    _drawCard(png, x0 + cardW + gap, y0, cardW, cardH, next, false, result === 'win');
  } else {
    const x0 = Math.round((W - cardW) / 2);
    _drawCard(png, x0, 72, cardW, cardH, current, false, true);
  }

  // Footer stats
  const line = streak > 0
    ? `STREAK ${streak}  ·  ${mult.toFixed(1)}X`
    : 'PICK HIGHER  LOWER  OR  SAME';
  _drawPixelLabel(png, line, W / 2, H - 36, 2, [160, 180, 220, 255]);

  if (result === 'win') {
    _drawPixelLabel(png, 'WIN', W / 2, H - 58, 2, [80, 230, 140, 255]);
  } else if (result === 'lose') {
    _drawPixelLabel(png, 'LOSE', W / 2, H - 58, 2, [255, 110, 110, 255]);
  }

  return PNG.sync.write(png);
}

/* ─── FIND THE QUEEN (3-card monte, progressive) ────────────────────────── */

function monteCardCount(level) {
  if (level <= 1) return 3;
  if (level === 2) return 4;
  if (level === 3) return 5;
  if (level === 4) return 6;
...
function monteMultiplier(streak) {
  // 1 correct → 2x, 2 → 4x, 3 → 8x, …
  if (streak <= 0) return 1;
  return Math.pow(2, streak);
}

/** Place the queen at a random index among `count` face-down cards. */
function monteDeal(count) {
  const queenIdx = _randInt(count);
  return { count, queenIdx };
}

/**
 * @param {number} count
 * @param {number|null} pickIdx  player pick, or null while choosing
 * @param {number|null} queenIdx reveal queen when set with reveal=true
 * @param {boolean} reveal
 * @param {number} level
 * @param {number} mult
 */
function renderMontePng(count, pickIdx = null, queenIdx = null, reveal = false, level = 1, mult = 2) {
  const W = Math.max(520, 40 + count * 70);
  const H = 300;
  const png = new PNG({ width: W, height: H });
  _flatBg(png, [14, 12, 22, 255]);
  _glassPanel(png, 14, 14, W - 28, H - 28, { radius: 18, fill: [28, 18, 42, 230], border: [160, 100, 220, 100] });

  _drawPixelLabel(png, 'FIND THE QUEEN', W / 2, 40, 2, [255, 220, 140, 255]);
  _drawPixelLabel(png, `LEVEL ${level}  ·  ${mult}X`, W / 2, 62, 2, [200, 170, 255, 255]);

  const cardW = 56, cardH = 84;
  const gap = 12;
  ...
  for (let i = 0; i < count; i++) {
    ...
    const isQueen = reveal && queenIdx === i;
    if (reveal) {
      if (isQueen) {
        // Queen face — gold edge
        _drawCard(png, x, y, cardW, cardH, { r: 'Q', s: '♥️' }, false, true);
        if (isPick) _drawPixelLabel(png, 'OK', x + cardW / 2, y + cardH + 18, 2, [80, 230, 140, 255]);
      } else {
        _drawCard(png, x, y, cardW, cardH, { r: 'A', s: '♠️' }, false, false);
        if (isPick) _drawPixelLabel(png, 'X', x + cardW / 2, y + cardH + 18, 2, [255, 100, 100, 255]);
      }
    } else {
      _drawCard(png, x, y, cardW, cardH, { r: '?', s: '♠️' }, true, isPick);
    }
    x += cardW + gap;
  }

  if (!reveal) {
    _drawPixelLabel(png, 'PICK A CARD', W / 2, H - 36, 2, [180, 160, 220, 255]);
  } else if (pickIdx === queenIdx) {
    _drawPixelLabel(png, 'QUEEN FOUND', W / 2, H - 36, 2, [80, 230, 140, 255]);
  } else {
    _drawPixelLabel(png, 'WRONG CARD', W / 2, H - 36, 2, [255, 110, 110, 255]);
  }
...
module.exports = {
  ...
  rankValue, hlMultiplier, hlNewDeck, drawCardFrom, hlResolve, renderHigherLowerPng,
  monteCardCount, monteMultiplier, monteDeal, renderMontePng,
};
