...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'sell') {
      ...
      // Selling is a coin transaction wearing a card-collection hat — with
      // the economy off there's nothing to pay a sale price in, so the
      // command says that plainly rather than pretending a sale happened.
      if (!isFeatureEnabled(interaction.guild?.id, 'economy')) {
        return interaction.reply({
          content: '💤 Selling is part of the coin economy, which is switched off for this server. Your cards are safe — keep collecting.',
