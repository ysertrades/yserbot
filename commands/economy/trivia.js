...
async function resolveAnswer(session, choice) {
  ...
  let reward = 0;
  // Trivia stays fun without the economy — the game keeps score by correct
  // answers either way, it just stops being worth coins on top of that.
  const economyOn = isFeatureEnabled(session.guildId, 'economy');

  if (correct) {
    session.correctCount++;
    if (economyOn) {
      reward = computeReward(session.correctCount - 1, session.econ);
  ...
  const resultEmbed = new EmbedBuilder()
    ...
    .setDescription(
      correct
        ? (economyOn ? `✅ **Correct!** +**${fmt(reward)}** coins` : '✅ **Correct!**')
        : choice === null
  ...
  if (isLast) {
    ...
    resultEmbed.addFields({
      ...
      value: economyOn
        ? `${session.correctCount}/${session.econ.questions} correct • **${fmt(session.totalCoins)}** coins earned\n💰 Balance: **${fmt(getBalance(session.userId))}** coins${boostNote(session.guildId) ? `\n${boostNote(session.guildId)}` : ''}`
        : `${session.correctCount}/${session.econ.questions} correct`,
    });
