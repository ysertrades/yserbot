...
if 'id="form-whop"' in html:
    ...
    block = """
      <div class=\"panel\" id=\"whop-panel\">
        ...
        <p class=\"muted\">API key once. Scan courses, pick which to track. New video lessons post here with the course banner. Optional role ping and link button below.</p>
...
"""
    ...
    if marker in html:
    ...
    elif 'data-section="economy"' in html:
        html = html.replace(
        ...
        raise SystemExit("Could not find Economy section to insert Whop panel")
...
js_path = Path("web/public/app.js")
...
assert "function renderWhop(" in js_path.read_text()
assert "renderWhop();" in js_path.read_text()
print("app.js OK")
PY
...
if "'whop.lesson'" in t:
  ...
  /* -- market news --------------------------------------------------------- */"""
    if needle not in t:
        print("WARN: could not add whop.lesson — check Appearance catalogue manually")
    else:
...
echo "Then restart the bot. Panel: Feeds → Whop courses"
