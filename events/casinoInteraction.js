...
async function applyResult(interaction, userId, updates) {
  ...
  await interaction.followUp({
    embeds: [new EmbedBuilder()
      ...
      .setFooter({ text: 'QuantLab Casino  •  Keep the streak alive!' })],
...
async function handleButton(interaction) {
  ...
  if (type === 'diceaccept') {
    ...
    const pvpEmbed = new EmbedBuilder()
      ...
      .setFooter({ text: 'QuantLab Casino  •  Table host can start a rematch' });
...
async function showBetSelection(interaction, game) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `QuantLab Casino  •  Min: ${fmt(settings.minBet)}  ·  Max: ${fmt(maxBet)}${isVip ? ' 👑' : ''}` });
...
async function showCoinflipChoice(interaction, s) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  50 / 50' });
  const row = new ActionRowBuilder().addComponents(
...
async function resolveCoinflip(interaction, s, choice) {
  ...
  const spinEmbed = new EmbedBuilder()
    ...
    .addFields({ name: '💸 Bet', value: `**${fmt(s.bet)}** coins`, inline: true })
    .setFooter({ text: 'QuantLab Casino' });
...
async function showRacePick(interaction, s) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Purely random — no predictable odds' });
...
async function renderBJ(interaction, s, state, initial = false) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Dealer hits soft 17  •  BJ pays 3:2' });
...
async function startTrading(interaction, s) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Pick direction' });
  const row = new ActionRowBuilder().addComponents(
...
async function showDiceModeSelect(interaction, s) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Odds are randomised each game' });
  const row = new ActionRowBuilder().addComponents(
...
async function showDicePvpChallenge(interaction, s) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  First to accept wins the duel slot' });
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
...
async function showRoulettePick(interaction, s) {
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Roulette controls are neutral/secondary for clarity' });
...
async function resolveRoulette(interaction, s) {
  ...
  const spinEmbed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Secure random spin in progress' });
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  European Roulette  •  Cryptographically secure number selection' });
...
async function resolveWheel(interaction, s) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: unlimited ? 'QuantLab Casino  •  Wheel of Fortune  •  ⏩ Cooldown Skip active — unlimited spins' : 'QuantLab Casino  •  Wheel of Fortune  •  10 spins/day' });
...
async function showHigherLowerPrompt(interaction, s) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Fast card calls' });
...
async function showMontePrompt(interaction, s, shuffling = false) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'QuantLab Casino  •  Find the Queen' });
