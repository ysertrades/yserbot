  ...
  async execute(interaction, client) {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.inGuild()) {
        const featureKey = groupForCommand(interaction.commandName);
        if (featureKey && !isFeatureEnabled(interaction.guild.id, featureKey)) {
          const label = FEATURE_GROUPS.find(g => g.key === featureKey)?.label || featureKey;
          return interaction.reply({
            embeds: [embedUtil.error('Feature Turned Off', `**${label}** is switched off for this server. An admin can turn it back on from the control panel's Settings tab.`)],
            flags: EPHEMERAL_FLAG,
          }).catch(() => {});
        }
      }
    ...
    if (interaction.isButton()) {
      ...
      try {
        ...
        // XP Ladder Desk — Refresh / Me on /leaderboard
        if (id.startsWith('lb:')) {
