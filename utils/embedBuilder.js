...
// Discord embeds only support a single solid sidebar colour — no true
// gradients. Every value here comes straight from QuantLab's "Phantom"
// palette (brand book, Style Guide · Color) by way of brandTheme.js's
// SEMANTIC map, so severity reads as depth of purple rather than a
// red/green swing — no neon P&L colours, per the brand's own guardrails.
// See brandTheme.js for the mapping and the rationale behind each one.
const { SEMANTIC } = require('./brandTheme');
const hexToInt = hex => parseInt(String(hex).replace('#', ''), 16);
const colors = Object.fromEntries(Object.entries(SEMANTIC).map(([k, v]) => [k, hexToInt(v)]));
...
// became a five-line card: an author row saying QuantLab, the line itself,
...
function guildLook(type, guild) {
  const fallback = { color: null, name: 'QuantLab', footer: `${guild?.name || 'Server'} • QuantLab` };
