...
async function resolveAppCourseIds(settings, entry) {
  const fromCatalog = (settings.catalog || [])
    .filter(c => c.experienceId === entry.id)
    .map(c => c.id);
...
async function newLessons(guildId) {
  ...
  for (const entry of s.log) {
    ...
    // Course-app tracker: never pass experience id to listLessons
    if (e.type === 'app') {
      const courseIds = await resolveAppCourseIds(s, e);
      if (!courseIds.length) {
        console.warn(`[WHOP] app ${e.id}: no courses in catalog/API — rescan or wait`);
        nextLog.push(e);
        continue;
      }
      const known = { ...(e.known || {}) };
      for (const cid of courseIds) {
        let lessons;
        try {
          lessons = await listLessons(s.apiKey, cid);
        } catch (err) {
        ...
        for (const lesson of lessons) {
          ...
          if (s.onlyVideos && String(lesson.lessonType || '').toLowerCase() === 'multi' && !lesson.videoReady && !lesson.pdfReady) {
            continue;
          }
          fresh.push(lesson);
        }
