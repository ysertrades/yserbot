...
function getSettings(guildId) {
  ...
  return {
    ...
    log,
    pollMinutes: Math.min(120, Math.max(2, Number(stored.pollMinutes) || 2)),
...
function lessonLink(settings, entry, lesson) {
  ...
  if (route) return `https://whop.com/${route}`;
...
async function addToLog(guildId, courseId, { channelId = null, mentionRoleId = null } = {}) {
  ...
  let entry = {
    ...
    experienceId: fromCatalog.experienceId || null, experienceName: fromCatalog.experienceName || null,
    channelId: channelId || null, mentionRoleId: mentionRoleId || null,
    known: {}, baselined: false, addedAt: Date.now(),
  };
  if (s.apiKey) entry = await baselineEntry(s.apiKey, entry);
  const log = [...s.log, entry];
  setSettings(guildId, { log, lastError: null, lastCheckAt: 0 });
...
async function addAppToLog(guildId, experienceId, { channelId = null, mentionRoleId = null } = {}) {
  ...
  let entry = {
    ...
    experienceId, experienceName: fromApps?.name || courses[0]?.experienceName || null,
    channelId: channelId || null, mentionRoleId: mentionRoleId || null,
    known: {}, baselined: false, addedAt: Date.now(),
  };
...
async function newLessons(guildId) {
  ...
  for (const entry of s.log) {
    ...
    if (e.type === 'app') {
      const courseIds = await resolveAppCourseIds(s, e);
      if (!courseIds.length) {
        console.warn(`[WHOP] app ${e.id}: no courses — rescan`);
        nextLog.push(e);
        continue;
      }
      const known = { ...(e.known || {}) };
