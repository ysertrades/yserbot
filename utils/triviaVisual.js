...
const {
  PNG, setPxBlend, hexToRgb,
  drawText, drawTextCentered, wrapText, GLYPH_H,
} = require('./pixelArt');
const { RGBA_DARK: DARK, darkCard, fillCanvas } = require('./brandTheme');

// Four brand tones stand in for the four categories — same family, each a
// different step of purple/cyan, rather than the traffic-light spread this
// used to be.
const CATEGORY_META = {
  trading:   { label: 'TRADING TRIVIA',   color: 0x4FB6E8 },
  economics: { label: 'ECONOMICS TRIVIA', color: 0x9397EE },
  politics:  { label: 'CIVICS TRIVIA',    color: 0x3A3F8F },
  sports:    { label: 'SPORTS TRIVIA',    color: 0xB7BAF3 },
};
...
function generateTriviaImage(data) {
  ...
  const meta = CATEGORY_META[category] || { label: category.toUpperCase(), color: 0x9397EE };
  ...
  for (const line of qLines) {
    drawTextCentered(png, line, W / 2, qy, 3, DARK.ink);
    qy += 3 * GLYPH_H + 10;
  }
  ...
  choices.forEach((choice, i) => {
    ...
    darkCard(png, cx0, cy0, cellW, cellH, { radius: 16, fill: DARK.raised, border: accent, borderAlpha: 0.4 });
    ...
    for (const line of choiceLines.slice(0, 3)) {
      drawText(png, line, cx0 + 44, cy, 2, DARK.ink);
      cy += 2 * GLYPH_H + 6;
    }
  });
